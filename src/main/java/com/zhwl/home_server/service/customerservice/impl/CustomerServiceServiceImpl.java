...
public class CustomerServiceServiceImpl implements CustomerServiceService {
    ...
    public Integer save(CustomerService customerService) throws BaseException {
        if(customerService.getIsFreeze() == null )
    ...
    */
    ...
    @Override
    public boolean addCustomerService(Map<String, Object> map) {
        ...
        CustomerService customerService = (CustomerService) map.get("customerService");
        validateCustomerService(username,password,customerService);
        ...
        customerService.setAddTime(new Date());
        customerService.setUpdateTime(customerService.getAddTime());
        customerService.setSysUserId(sysUser.getId());
        Integer csSave = save(customerService);
        if(1 != csSave) throw new BaseException("客户信息保存失败");
        return true;
    }
    public void validateCustomerService(String username,String password,CustomerService customerService){
        if(Strings.isNullOrEmpty(username) || username.trim().length()<6 || username.trim().length() >20)
            throw new BaseException("用户名不能为空、长度在6-20位之间");
        username = username.trim();
        if(Strings.isNullOrEmpty(password) || password.trim().length()<6 || password.trim().length() >20)
            throw new BaseException("密码不能为空、长度在6-20位之间");
        password = password.trim();
        if(null == customerService)
            throw new BaseException("客服信息不能为空");
        if(null == customerService.getName())
            throw new BaseException("客服名字不能为空");
